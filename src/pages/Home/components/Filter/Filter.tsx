...
const Filter = (props: any) => {
  ...
  const initSupplyOptions = useMemo(() => {
    ...
    const defaultOptions =
      props?.filters?.supplies.length > 0
        ? Object.entries(grouped).reduce(
            (filtered: IComplexSelectData[], option) => {
              const [, values] = option;
  ...
  const handleSupplyCategoriesSelected = (
    supplyCategoriesSelected: readonly IComplexSelectData[]
  ) => {
    const grouped = group(result.data ?? [], 'supplyCategory.name');

    const supplyOptionsFiltered = Object.entries(grouped).reduce(
      (filtered: IComplexSelectGroupedData[], option) => {
    ...
    setFieldValue(
      'supplyCategories',
      supplyCategoriesSelected.map((el) => el.value)
    );
    setSupplyOptions(supplyOptionsFiltered);
  };

  const handleSupplySelected = (
    supplySelected: readonly IComplexSelectData[]
  ) => {
    setFieldValue(
      'supplies',
      supplySelected.map((el) => el.value)
    );
  };

  const handlePriorityOptionSelected = (
    priorityOptionSelected: IComplexSelectData
  ) => {
            ...
            <div className="flex flex-col gap-2 w-full my-4">
              ...
              <div className="flex flex-col gap-1 w-full">
                ...
                <Select
                  defaultValue={
                    props.filters.priority
                      ? priorityOptions
                          .filter(
                            (el) =>
                              props.filters.priority === parseInt(el.value)
                          )
                          .map((element) => {
                            return {
                              label: element.label,
                  ...
                  onChange={(priorityOptionsSelected: any) =>
                    handlePriorityOptionSelected(priorityOptionsSelected)
                  }
                />
              </div>
              <div className="flex flex-col gap-1 w-full">
                ...
                <Select
                  defaultValue={
                    props.filters.supplyCategories
                      ? supplyCategories
                          .filter((el) =>
                            props.filters.supplyCategories.some(
                              (suplyCategoryId: string) =>
                                suplyCategoryId === el.id
                            )
                          )
                          .map((element) => {
                            return {
                  ...
                  name="colors"
                  options={
                    supplyCategories.map((element: ISupplyCategory) => {
                      return {
                        label: element.name,
                        value: element.id,
                      };
                  ...
                  classNamePrefix="select"
                  onChange={(supplyCategoriesSelected) =>
                    handleSupplyCategoriesSelected(supplyCategoriesSelected)
                  }
                />
              </div>
              <div className="flex flex-col w-full">
                ...
                <Select
                  ...
                  onChange={(suppliesSelected) =>
                    handleSupplySelected(suppliesSelected)
                  }
                />
              </div>
            </div>
              ...
              <div>
                <label className="flex items-center mb-4">
                  <input
                    ...
                    type="checkbox"
                    className="mr-2 w-4 h-4 text-dark-green bg-gray-100 border-gray-300 rounded focus:dark-green dark:focus:white dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                    onChange={() =>
                      setFieldValue(
                        'filterAvailableShelter',
                        !values.filterAvailableShelter
                      )
                    }
                  ...
                  <input
                    ...
                    onChange={() =>
                      setFieldValue(
                        ...
                        !values.filterUnavailableShelter
                      )
                    }
                  ...
                  <input
                    ...
                    onChange={() =>
                      setFieldValue(
                        ...
                        !values.waitingShelterAvailability
                      )
                    }
